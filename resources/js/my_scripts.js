...
function loadStatsPage(){
	...
	for (row_counter = 0; row_counter < table.rows.length; row_counter++) {
		for (col_counter = 0; col_counter < table.rows[row_counter].cells.length; col_counter++) {
			...
			var match = cell_value.match(regex);
			if(match && match.index==0)
			{
				console.log(table.rows[row_counter].cells[col_counter].innerHTML);
				counter++;
				if ((counter % 2)==1) {
					tempHomeScore=cell_value;
				}
				else {
					tempOppScore=cell_value;
					if (tempOppScore > tempHomeScore){
						losses++;
						table.rows[row_counter].cells[col_counter+1].innerHTML =table.rows[row_counter].cells[col_counter-2].innerHTML
					}
					else if (tempHomeScore > tempOppScore) {
						wins++;
						table.rows[row_counter].cells[col_counter+1].innerHTML ="CU Boulder"
					}
					else {
						table.rows[row_counter].cells[col_counter+1].innerHTML ="Tie"
					}//else
				}//else
			}//if
		}//for
...
function switchPlayers(playerNum) {
	...
	avg_rec_yards.innerHTML = (Math.round(player.receiving_yards/player.games_played));
...
function loadPlayersPage() {
	...
	var dropdown = document.getElementById("player_selector").innerHTML;
	console.log(dropdown);
	var anchorTags="";
	for (playersCounter; playersCounter < players.length; playersCounter++) {
		anchorTags +='<a class="dropdown-item" href="#" onclick="switchPlayers(' + playersCounter + ')">'+players[playersCounter].name+'</a>';
	}
	document.getElementById("player_selector").innerHTML = anchorTags;
}
